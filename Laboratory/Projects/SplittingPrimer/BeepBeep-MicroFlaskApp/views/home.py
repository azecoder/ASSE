...
def index():
    ...
    if current_user is not None and hasattr(current_user, 'id'):
        ...
        if challenged_run_reply:
            ...
            ch_run = last_run = requests.get(DATASERVICE + '/users/' + str(current_user.id) + '/runs/' + str(challenged_run['run_id'])).json()

            last_run_id = requests.get(DATASERVICE + '/users/' + str(current_user.id) + '/runs/getMaxId').json()['max_id']

            last_run = requests.get(DATASERVICE + '/users/' + str(current_user.id) + '/runs/' + str(last_run_id)).json()
