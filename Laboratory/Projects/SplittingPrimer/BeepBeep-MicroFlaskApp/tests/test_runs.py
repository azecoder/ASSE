...
def test_single_run(client, requests_mock):
	with UserContext(client,requests_mock) as uc:
		response = client.get("/runs/1")
		assert response.status_code == 200


def test_home_runs(client, requests_mock):
	with UserContext(client, requests_mock) as uc:
...
def test_single_run_view(client, requests_mock):
	with UserContext(client,requests_mock) as uc:
		response = client.get("/runs/1")
...
def test_bad_singlerun(client, requests_mock):
	with UserContext(client,requests_mock) as uc:
